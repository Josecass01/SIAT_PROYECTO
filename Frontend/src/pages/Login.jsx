      ...
      <div className="flex-grow flex flex-col items-center justify-start pt-12 px-4">
        <div className="bg-white rounded-xl shadow-lg w-full max-w-md">
          ...
          <form className="px-6 py-8" onSubmit={handleSubmit}>
            ...
            <div className="mt-4"><label htmlFor="email">Correo Electronico <span className="text-red-500">*</span></label><input type="email" name="email" value={email} onChange={(e) => setEmail(e.target.value)} required className="block w-full rounded-md border-gray-300"/></div>
            <div className="mt-4"><label htmlFor="password">Contraseña <span className="text-red-500">*</span></label><input type="password" name="password" value={password} onChange={(e) => setPassword(e.target.value)} required className="block w-full rounded-md border-gray-300"/></div>
            <div className="mt-6"><button type="submit" disabled={loading} className="w-full flex justify-center py-2 px-4 border-transparent rounded-md shadow-sm text-white bg-sky-500 hover:bg-sky-600 disabled:bg-sky-400">{loading ? 'Iniciando sesión...' : 'Log In'}</button></div>
          </form>
        </div>
      </div>
    </div>
