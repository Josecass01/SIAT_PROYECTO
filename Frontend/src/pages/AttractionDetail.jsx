...
export default function AttractionDetail() {
  ...
  const handleDelete = async () => { if (window.confirm('¿Estás seguro de que quieres eliminar esta atracción?')) { try { await api.delete(`/attractions/${id}`); navigate('/places'); } catch (err) { alert('No se pudo eliminar la atracción.'); } } };
  const handleReviewSubmit = async (e) => { e.preventDefault(); setReviewLoading(true); setReviewError(null); try { await api.post(`/attractions/${id}/reviews`, { rating, comment }); setReviewLoading(false); setRating(0); setComment(''); alert('¡Reseña enviada con éxito!'); fetchAttraction(); } catch (err) { setReviewError(err.response?.data?.message || 'Error al enviar reseña.'); setReviewLoading(false); } };
  ...
  const handleDeleteReview = async (reviewId) => { if (window.confirm('¿Estás seguro de que quieres eliminar esta reseña?')) { try { await api.delete(`/attractions/${id}/reviews/${reviewId}`); fetchAttraction(); } catch (error) { alert('No se pudo eliminar la reseña.'); } } };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-lg shadow p-6">
              <h3 className="text-xl font-semibold text-gray-800 border-b pb-2 mb-4">Detalles y Calificaciones</h3>
              <div className="grid grid-cols-2 gap-6">
                ...
                <div><h4 className="font-semibold text-gray-600">Dirección</h4><p>{attraction.ubicacion_texto || 'No especificada'}</p></div>
                ...
                <div className="col-span-2 border-t mt-4 pt-4">
                  <h4 className="font-semibold text-gray-600 mb-2">Calificación de la Entidad</h4>
                  <ul className="list-disc list-inside space-y-1 text-sm">
                    <li>Estado de Conservación: <strong>{attraction.calificacion_entidad?.estadoConservacion || 0}/21</strong></li>
                    <li>Constitución del Bien: <strong>{attraction.calificacion_entidad?.constitucionDelBien || 0}/21</strong></li>
                    <li>Representatividad General: <strong>{attraction.calificacion_entidad?.representatividadGeneral || 0}/28</strong></li>
                  </ul>
                </div>
              </div>
            </div>
